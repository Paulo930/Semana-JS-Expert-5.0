export default class AppController {
  ...
  async onProgress({ processedAlready, filename }) {
    const file = this.uploadingFile.get(filename);
  ...
  updateProgress(file, percent) {
    const uploadingFiles = this.uploadingFile;
  ...
  async onFileChange(files) {
    ...
    // você fazer outro upload, ele vai fechar o modal e inicial do zero
    this.uploadingFile.clear();
    ...
    for (const file of files) {
      this.uploadingFile.set(file.name, file);
